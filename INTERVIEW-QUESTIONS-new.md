...
19. [## 🏗️ Real-World Microservices Architecture Examples
...
**Key Benefits:**
- ✅ API responds in **50ms** (doesn't wait for Kafka, email, or downstream services)
- ✅ If Kafka is down, database still saves (user not lost)
- ✅ Event consumers can retry independently
    ...
    public void handleUserCreatedEvent(UserCreatedEvent event) {
        ...
        if (anomalyDetectionAgent != null) {
            ...
            if ("QUARANTINE".equals(anomalyDecision.getAction())) {
                ...
                logger.info("⚠️ User flagged for review: {}", event.getUsername());
...
- **Consumer Threads**: 15 total (5 services × 3 threads each)
...
**Benefits:**
- ✅ **Independent scaling**: Scale evaluation_db separately from sampling_db
- ✅ **Technology flexibility**: Can use PostgreSQL for evaluation, MongoDB for evidence
- ✅ **Fault isolation**: sampling_db crash doesn't affect evaluation-service
- ✅ **Independent deployments**: Change evaluation schema without coordinating with sampling

**Challenges & Solutions:**
- ❌ **No JOIN across services** → ✅ Use Kafka events + denormalization
- ❌ **Distributed transactions** → ✅ Use Saga pattern (see below)
- ❌ **Data consistency** → ✅ Eventual consistency + compensating transactions
...
**What I learned:**
⚠️ **Eventual Consistency**: Hard to debug when data is out of sync across services
⚠️ **Distributed Tracing**: Need correlation IDs to trace requests across services
⚠️ **Testing**: Integration tests complex with 5 services + Kafka + PostgreSQL
⚠️ **Monitoring**: Need centralized logging (we use CloudWatch with correlation IDs)
...
## ☕ Java Fundamentals - Threads & Concurrency
...
```java
// ❌ BAD: No multiple inheritance, tightly coupled
...
```java
// ❌ BAD: Locks entire object, blocks all synchronized methods
...
❌ Don't nest locks (can cause deadlock)
❌ Don't call external methods inside synchronized blocks
❌ Don't use `synchronized` for long-running operations"
...
**Don'ts:**
❌ Don't use `Executors.newCachedThreadPool()` for unbounded tasks
❌ Don't forget to shutdown - causes thread leaks
❌ Don't submit blocking I/O tasks to CPU-bound pools
❌ Don't ignore `RejectedExecutionException`"
...
"**1. Volatile Keyword**
```java
// ❌ BAD: Race condition, threads may cache value
...
```java
// ❌ BAD: Deadlock - Thread 1 locks A then B, Thread 2 locks B then A
...
```java
// ❌ BAD: Synchronized overhead, legacy class
...
// ❌ BAD in multi-threaded environment
...
**Don'ts:**
❌ Don't synchronize entire HashMap - use ConcurrentHashMap
❌ Don't use Hashtable (legacy, slow)
❌ Don't assume iteration order in HashMap
❌ Don't put null keys in ConcurrentHashMap"
...
**❌ BAD: Manual Dependency Creation (Tight Coupling)**
```java
public class UserService {
...
```java
// ❌ BAD: Can't test without Spring, mutable fields
@Service
public class UserService {
    @Autowired
...
✅ Reusability - share components across application
...
```java
// ✅ Generic component, no specific role
@Component
...
```java
// ✅ Repository - database operations, exception translation
@Repository
...
```java
// ✅ Controller - handles requests, returns view names
@Controller
...
| @Repository | Data | Database access | Yes (SQL → DataAccessException) |
...
**✅ Better: Use Specific Annotations**
```java
@RestController
@RequestMapping("/api/users")
public class UserController {
...
// ❌ BAD: CascadeType.ALL on ManyToMany (can cause issues)
@ManyToMany(cascade = CascadeType.ALL) // Don't do this!
private Set<Course> courses;

// ✅ GOOD: Specific cascades for ManyToMany
@ManyToMany(cascade = {CascadeType.PERSIST, CascadeType.MERGE})
private Set<Course> courses;
```
...
// ✅ BEST: Use LAZY and fetch explicitly when needed
...
**In my project:**
- Account Service → Transaction Service: Synchronous REST (need balance immediately)
- Order Service → Notification Service: Async SNS/SQS (can wait for email)
- All external clients → API Gateway → Services
...
- ❌ If Account Service changes, Transaction Service breaks
...
- ⚠️ In maintenance mode (Spring recommends WebClient)
...
**When to use:**
- ✅ Legacy Spring Boot projects (already using it)
...
**When to use:**
- ✅ **NEW Spring Boot projects (highly recommended)**
- ✅ High-traffic applications (10,000+ req/sec)
- ✅ Need to call multiple APIs in parallel
...
**When to use:**
- ✅ **Microservices calling each other (best choice)**
- ✅ Want declarative style (less code)
- ✅ Need built-in Circuit Breaker
...
| **Spring Recommendation** | ⚠️ Maintenance | ✅ Preferred | ✅ Good |
...
- Used **Feign Client** for Order Service → Inventory Service
...
```
For NEW projects:
├── Internal microservices communication → Feign Client (easiest)
├── High-traffic APIs (>1000 req/sec) → WebClient (fastest)
├── Parallel API calls → WebClient (non-blocking)
└── Simple REST calls → WebClient (future-proof)

For EXISTING projects:
├── Already using RestTemplate? → Keep it (don't refactor unless needed)
└── Performance issues? → Migrate to WebClient
```"

---

## ⚡ Async Programming & @Async Annotation
...
- ❌ Thread is blocked (wasted)
- ❌ Under load, threads exhausted
...
**Synchronous:**
```
Thread-1: [Create Order]─[Send Email───2s───]─[Send SMS─1s─]─[Response] = 3s
Thread-2:                                        [Create Order]─[Send Email...
...
**Asynchronous:**
```
Thread-1: [Create Order]─[Response]─[Create Order]─[Response] = <100ms each
Thread-2: [Create Order]─[Response]─[Create Order]─[Response]
Async Pool:
  Thread-A: [Send Email───2s───]─[Send SMS─1s─]
  Thread-B: [Send Email───2s───]─[Send SMS─1s─]
...
**@Transactional with @Async (⚠️ DANGEROUS):**

```java
// ❌ BAD: Both annotations on same method
@Async
@Transactional
public void processPayment(Payment payment) {
    // PROBLEM:
...
public class NotificationService {
    ...
    public void processOrder(Order order) {
        saveOrder(order);
        sendEmail(order);  // ❌ @Async doesn't work!
    }
...
**Why it doesn't work:**
```
Client → Spring Proxy (@Async logic) → NotificationService.processOrder()
                ↓
...
public class OrderProcessingService {
    ...
    public void processOrder(Order order) {
        saveOrder(order);
        emailService.sendEmail(order);  // ✅ Works! Goes through proxy
    }
}
...
**Now it works:**
```
OrderProcessingService → Spring Proxy (@Async logic) → EmailAsyncService.sendEmail()
                                ↓
...
public class PaymentService {
    ...
    public void processPayment(Payment payment) {
        ...
        savePayment(payment);  // ❌ @Transactional doesn't work!
    }
...
public class PaymentService {
    ...
    public void processPayment(Payment payment) {
        validatePayment(payment);
        transactionService.savePayment(payment);  // ✅ Works!
    }
}
...
**⚠️ SPECIAL CASE: Both Methods Have @Async**
...
**What happens:**
1. `processNotifications()` runs in async thread ✅ (called from outside)
2. Inside `processNotifications()`, calls `sendEmail()` directly
3. `sendEmail()`'s @Async is **IGNORED** ❌
4. `sendEmail()` runs in the **SAME async thread**, NOT a new thread

**Key Point:**
- ✅ `processNotifications()` is async (because called from outside class)
- ❌ `sendEmail()` is NOT async (because called from inside same class)
- Both methods run in the **same thread**

---

**⚠️ SPECIAL CASE: Both Methods Have @Transactional**
...
**What happens:**
1. `createOrder()` starts Transaction A ✅
2. Inside `createOrder()`, calls `saveOrder()` directly
3. `saveOrder()`'s @Transactional is **IGNORED** ❌
...
```java
// ❌ BAD: Everything in one service (self-invocation)
@Service
public class BadOrderService {
    
    public OrderResponse createOrder(OrderRequest request) {
        validateOrder(request);  // Normal method
        saveOrder(request);      // ❌ @Transactional ignored!
        sendNotifications();     // ❌ @Async ignored!
        updateInventory();       // ❌ @Async ignored!
...
// ✅ GOOD: Separated services (no self-invocation)
@Service
public class OrderService {
    @Autowired
    ...
    public OrderResponse createOrder(OrderRequest request) {
        ...
        // All these go through Spring proxies ✅
...
- ✅ @Async and @Transactional work correctly
...
|----------|--------|--------|
| Method A (no annotation) calls Method B (@Async) in **same class** | ❌ | Proxy bypassed |
| Method A (no annotation) calls Method B (@Async) in **different class** | ✅ | Goes through proxy |
| Method A (@Async) calls Method B (@Async) in **same class** | ⚠️ Partial | A is async, B is not |
| Method A (@Async) calls Method B (@Async) in **different class** | ✅ | Both async |
| Method A (@Transactional) calls Method B (@Transactional REQUIRES_NEW) in **same class** | ❌ | B's settings ignored |
| Method A (@Transactional) calls Method B (@Transactional REQUIRES_NEW) in **different class** | ✅ | B creates new transaction |
...
│                                                       │
│  account-service: [10.0.1.10, 10.0.1.11, 10.0.1.12] │
│  payment-service: [10.0.2.20, 10.0.2.21]            │
│  order-service: [10.0.3.30, 10.0.3.31, 10.0.3.32]   │
└──────────────────────────────────────────────────────┘
         ↑ Register              ↓ Query
         │                       │
    ┌─────────┐           ┌─────────┐
...
Sends heartbeat every 30 seconds
        ↓
If no heartbeat → Marked unhealthy
```
...
- Response 200 → Healthy (includes in load balancing)
- Response 503 or timeout → Unhealthy (excludes from load balancing)
...
|--------------------------|------------------------|
| ❌ Hardcoded IPs in config | ✅ Dynamic service resolution |
| ❌ Manual updates on scale | ✅ Auto-detects new instances |
| ❌ Traffic to dead instances | ✅ Only healthy instances |
| ❌ No load balancing | ✅ Built-in load balancing |
| ❌ Single point of failure | ✅ Redundancy across instances |
...
Auto-scaling adds 20 new instances:
  account-service: 3 → 10 instances
  payment-service: 2 → 8 instances
  order-service: 5 → 15 instances
        ↓
...
```
Mobile App needs:
├── User info → http://user-service:8081/api/users
├── Orders → http://order-service:8082/api/orders
├── Products → http://product-service:8083/api/products
├── Cart → http://cart-service:8084/api/cart
└── Payments → http://payment-service:8085/api/payments

Problems:
❌ Mobile app knows about 5 different services
❌ 5 different base URLs to manage
❌ Each service needs authentication logic
❌ CORS issues (different origins)
❌ No centralized rate limiting
❌ Hard to change service URLs
```
...
| **Scope** | External traffic (clients → services) | Internal traffic (service → service) |
...
// API Gateway:
1. Validates JWT token → Extracts user ID
2. Checks rate limit → User hasn't exceeded limit
3. Routes to order-service → http://order-service:8082/api/orders/123
...
// Benefits:
✅ Mobile app only knows about api.example.com
✅ Authentication centralized
✅ Rate limiting enforced
✅ Easy to add new services (just update Gateway routes)
✅ Can redirect traffic for A/B testing
```
...
- If electrical fault detected → Cut power immediately
- Prevents fire/damage
- After some time → Try again (reset)
...
**Impact:**
- 10 requests/sec × 30 sec timeout = 300 threads blocked
- Thread pool exhausted in 30 seconds
- Domino effect: 1 service failure → All services fail
...
│  (Failing)  │  Immediately return error (don't call service)
└──────┬──────┘
       │ Wait 60 seconds
       ↓
┌─────────────┐
...
Request 3 → Payment Service → Success ✅
Request 4 → Payment Service → Success ✅
Request 5 → Payment Service → Success ✅
...
```java
Request 1 → Payment Service → Success ✅
Request 2 → Payment Service → FAIL ❌
Request 3 → Payment Service → FAIL ❌
Request 4 → Payment Service → Success ✅
Request 5 → Payment Service → FAIL ❌
Request 6 → Payment Service → FAIL ❌
...
```java
Request 7 → Circuit Breaker blocks call ⛔
            Returns fallback immediately (no timeout!)
Request 8 → Circuit Breaker blocks call ⛔
Request 9 → Circuit Breaker blocks call ⛔
...
Request 11 → Payment Service → Success ✅
Request 12 → Payment Service → Success ✅
...
**Flow:**
```
Request 1 → Try → Fail → Retry (1s) → Fail → Retry (1s) → Fail
            ↓
Circuit Breaker opens (after multiple failures)
            ↓
Request 2 → Circuit OPEN → Fallback immediately (no retries)
Request 3 → Circuit OPEN → Fallback immediately
Request 4 → Circuit OPEN → Fallback immediately
```
...
|------------------------|---------------------|
| ❌ Threads blocked waiting | ✅ Fail fast (no waiting) |
| ❌ Cascading failures | ✅ Isolated failures |
| ❌ Entire system down | ✅ Other services keep running |
| ❌ 30s timeout per request | ✅ Instant fallback response |
...
**Benefits:**
- ✅ Payment service failures don't exhaust main thread pool
- ✅ Order processing continues even if payments are slow
...
- ✅ Failing fast (don't wait for timeouts)
